local Component = require("nui-form.component")
...
function Select:initial_state()
    return fn.ireduce(self:get_props().default_value, function(acc, id)
        local selected = fn.ifind(self.items, function(item)
            return item.id == id
        end)
...
function Select:__set_items(data)
    self.items = fn.ireduce(data, function(acc, item, index)
        table.insert(
            acc,
            type(item) == "string" and select_item(item, { id = index, value = item.value })
                or select_item(item.text, { id = item.id, value = item.value })
        )
        return acc
    end, {})
end
...
function Select:__extend_props()
    local props = {
        prepare_item = function(node)
            ...
            local is_selected = fn.isome(self:get_state(), function(item)
        ...
        on_select = function()
            ...
            if self:get_props().multiselect then
                local index = fn.find_index(current_state, function(node)
