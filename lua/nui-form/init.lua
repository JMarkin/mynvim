...
local TextInput = require("nui-form.text-input")
local Select = require("nui-form.select")
local Footer = require("nui-form.footer")

local fn = require("funcs")
local validators = require("nui-form.validators")
...
function NuiForm:__set_trigger_values_for_reload()
    self.__trigger_values = fn.imap(self.components, function(component)
...
function NuiForm:__calculate_total_height()
    self.total_height = fn.ireduce(self.components, function(acc, component)
...
function NuiForm:__determine_focusable_components()
    self.focusable_components = fn.ifilter(self.components, function(component)
        return component:is_focusable() and not component:is_hidden()
    end)

    fn.ieach(self.focusable_components, function(component, index)
...
vim.api.nvim_create_user_command("OpenNuiForm", function(opts)
